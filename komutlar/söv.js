exports.run = async (client, msg, args) => {
 ...
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Sen Kimsin Sahibime Sövüyon Amına Kodumun Jedayı senı sıkerımm :).')
}})
  if(member.id === "335413922649604096")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Birşeyler ters gitti @NetWork#0757  etiketlemişsin yarram!`)
}})
  if(member.id === "203116573211230208")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Yapımcım @NetWork#0757 sövemezsin amını sikerim.`)
}})
