 ...
 channel.setParent(message.guild.channels.find(channel => channel.name === "✮ ▬  ▬ Duyuru Kanalları▬  ▬ ✮")));
  ...
  .then(c => {
    ...
    c.overwritePermissions(role, {
       ...
       message.guild.owner.send("Gerekli Herşey Kuruldu Rahatına Bak aslanım! **ykisecbet  Bot'un kıymetini bil :D**")
              ...
              if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {
                    ...
                    return msg.channel.send(`${msg.author.tag}, **Reklam Yapmak Yasak bırdaha reklam yaparsan sikerim!**`).then(msg => msg.delete(25000));
                  }             
                } catch(err) {
                  console.log(err);
                }
